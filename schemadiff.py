# copied from migrate.versioning.schemadiff in order to add more features.
...
class SchemaDiff(object):
    ...
    :param verify_tables:
      List of non-model table names to verify presence of.

    :param table_verify:
      Dict of table name to non-model column name to verify presence of.
    ...
    def __init__(self,
        ...
        verify_tables = set(verify_tables or [])
        table_verify = table_verify or {}
        ...
        self.tables_missing_from_A = sorted(
            B_table_names - verify_tables - A_table_names - excludeTables
            )
        self.tables_missing_from_B = sorted(
            (A_table_names | verify_tables) - B_table_names - excludeTables
            )
        ...
        for table_name in A_table_names.intersection(B_table_names):
            ...
            verify_extras = set(table_verify.get(table_name, []))
#            if verify_extras:
#                from util.log import logger
#                logger.error('xxx %s' % verify_extras)

            A_column_names = set(A_table.columns.keys())
            B_column_names = set(B_table.columns.keys())

            td.columns_missing_from_A = sorted(
                B_column_names - verify_extras - A_column_names
                )

            td.columns_missing_from_B = sorted(
                (A_column_names | verify_extras) - B_column_names
                )
